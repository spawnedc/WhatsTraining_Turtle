    ...
    if categoryHasSpells then
      ...
      for spellIndex, categorySpell in ipairs(categorySpells) do
        ...
        row:SetScript("OnEnter", function()
          ...
          local name, rank, textureFile = SpellInfo(row.spell.id)
          self.tooltip:AddDoubleLine(name, rank, 1, 1, 1, 1, 1, 1)
          self.tooltip:AddLine(row.spell.id)
          self.tooltip:AddLine(textureFile)
        ...
        row:SetScript("OnMouseUp", function() Utils.log(self.spell.id) end)
